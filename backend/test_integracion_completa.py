...
def test_integracion_completa():
    ...
    print("\n TEST: Cargar Modelo de Sentimientos")
    try:
    ...
    print("\n TEST: Servicio de Análisis con Nivel de Confianza")
    try:
    ...
    print("\n TEST: DTOs con Campo confidence_level")
    try:
    ...
    print("\n TEST: Integración con Endpoint de API")
    try:
        ...
        if not hasattr(response, 'confidence_level'):
            warnings.append(" Respuesta API no incluye confidence_level en el objeto")
        else:
        ...
        warnings.append(f" No se pudo probar endpoint completo: {e}")
        print(f" Prueba de endpoint omitida (no crítico)")

    # TEST 5: Función interpretar_confianza
    print("\n TEST: Función interpretar_confianza")
    try:
    ...
    print("\n TEST: Metadata y Métricas del Modelo")
    try:
        if hasattr(model, 'metadata') and model.metadata:
            ...
            if 'test_metrics' in metadata:
                ...
                if accuracy < 0.70:
                    warnings.append(f" Accuracy es baja: {accuracy:.1%} (mínimo recomendado: 75%)")

                if 'per_class' in metrics:
                    print(f" Métricas por clase disponibles: ")
                else:
                    warnings.append(" No hay métricas por clase en metadata")
            else:
                warnings.append(" No hay métricas de test en metadata")
        else:
            warnings.append(" Modelo no tiene metadata")
    except Exception as e:
        warnings.append(f" Error leyendo metadata: {e}")

    # TEST 7: Prueba End-to-End
    print("\n TEST: Flujo End-to-End Completo")
    try:
